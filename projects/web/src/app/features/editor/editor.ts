...
@Component({
  ...
  <div class="editor-layout">
    <div class="main-col">
      <div class="toolbar">
        ...
        <button (click)="layout()">Auto-Layout</button>
        <button (click)="undo()" [disabled]="!canUndo()">Undo</button>
        <button (click)="redo()" [disabled]="!canRedo()">Redo</button>
        <button (click)="save('manual')">Guardar</button>
    ...
    <div class="side-col">
      <ng-container [ngSwitch]="selectedType()">
        <div *ngSwitchCase="'node'">
          <h3>Clase</h3>
          <form [formGroup]="nodeForm" (ngSubmit)="applyNodeForm()">
            <label>Nombre
              <input formControlName="name" />
            </label>
            <label>Estereotipo
              <input formControlName="stereotype" placeholder="entity, valueObject, etc." />
            </label>

            <h4>Atributos</h4>
            <div formArrayName="attrs" class="attrs">
              <div class="attr-row" *ngFor="let fg of attrsFA().controls; let i = index" [formGroupName]="i">
                <input formControlName="name" placeholder="nombre" />
                <input formControlName="type" placeholder="tipo (string,int,uuid…)" />
                <label><input type="checkbox" formControlName="pk" /> PK</label>
                <label><input type="checkbox" formControlName="unique" /> Unique</label>
                <label><input type="checkbox" formControlName="nullable" /> Nullable</label>
                <button type="button" (click)="removeAttr(i)">🗑</button>
              </div>
            </div>
            <button type="button" (click)="addAttr()">+ Atributo</button>

            <div class="mt">
              <button type="submit" [disabled]="nodeForm.invalid">Aplicar cambios</button>
            </div>
          </form>
        </div>
        ...
        <div *ngSwitchCase="'link'">
          <h3>Relación</h3>
          <form [formGroup]="linkForm" (ngSubmit)="applyLinkForm()">
            <label>Tipo
            ...
            <label>Card. origen (from)
              <input formControlName="fromCard" placeholder="1, 0..1, N, 1..N" />
            </label>
            <label>Card. destino (to)
              <input formControlName="toCard" placeholder="1, 0..1, N, 1..N" />
            </label>
          ...
          <p class="hint">Sugerencia: si es N→1, se agregará automáticamente un FK en la clase origen.</p>
        </div>

        <div *ngSwitchDefault>
          <h3>Selecciona una clase o relación</h3>
          <p class="muted">Haz clic en una clase para editar sus atributos, o en una relación para editar su tipo y cardinalidades.</p>
        </div>
      </ng-container>
    </div>
  </div>
  `,
  styles: [`
    .editor-layout{display:grid; grid-template-columns:1fr 320px; gap:12px; height:calc(100vh - 80px)}
    .main-col{display:flex; flex-direction:column}
    .diagram{flex:1; border:1px solid #e5e7eb; border-radius:12px}
    .toolbar{display:flex; gap:.5rem; align-items:center; margin-bottom:.5rem}
    .toolbar .muted{margin-left:auto; color:#6b7280}
    .side-col{border:1px solid #e5e7eb; border-radius:12px; padding:12px; overflow:auto}
    label{display:block; margin:.4rem 0}
    input, select{width:100%; padding:.4rem .5rem; border:1px solid #d1d5db; border-radius:8px}
    .attrs{display:flex; flex-direction:column; gap:.25rem}
    .attr-row{display:grid; grid-template-columns: 1fr 1fr auto auto auto 32px; gap:.25rem; align-items:center}
    .mt{margin-top:.6rem}
...
export class ModelEditorComponent implements OnInit, OnDestroy {
  ...
  attrsFA(): FormArray { return this.nodeForm.get('attrs') as FormArray; }
  private makeAttrArray(items: EntityAttr[]) {
    return items.map(a => this.fb.group({
  ...
  private initDiagram() {
    ...
    const attrTemplate =
      $(go.Panel, 'TableRow',
        ...
        $(go.Shape, 'Rectangle', { column: 0, width: 8, height: 8, strokeWidth: 0, margin: 2 },
          new go.Binding('fill', '', (a: any) => a.pk ? '#111827' : (a.unique ? '#6b7280' : '#e5e7eb'))),
        $(go.TextBlock, { column: 1, editable: true, margin: new go.Margin(2, 4, 2, 2) },
          new go.Binding('text', 'name').makeTwoWay()),
        $(go.TextBlock, { column: 2, editable: true, margin: new go.Margin(2, 4, 2, 2), stroke: '#6b7280' },
    ...
    diag.nodeTemplate =
      $(go.Node, 'Auto',
        ...
        $(go.Shape, 'RoundedRectangle', { fill: '#fff', stroke: '#d1d5db', strokeWidth: 1, parameter1: 8, portId: '', fromLinkable: true, toLinkable: true }),
        $(go.Panel, 'Table', { minSize: new go.Size(200, NaN), defaultRowSeparatorStroke: '#e5e7eb' },
          $(go.Panel, 'TableRow', { background: '#f9fafb' },
            $(go.TextBlock, { margin: 6, row: 0, column: 0, columnSpan: 3, font: 'bold 12pt sans-serif', editable: true },
              new go.Binding('text', 'name').makeTwoWay())
          ),
          $(go.TextBlock, { row: 1, column: 0, columnSpan: 3, margin: new go.Margin(0, 6, 4, 6), stroke: '#6b7280', editable: true },
            new go.Binding('text', 'stereotype').makeTwoWay()),
          $(go.Panel, 'Table', { row: 2, column: 0, itemTemplate: attrTemplate },
    ...
    diag.linkTemplate =
      $(go.Link, { routing: go.Link.AvoidsNodes, corner: 8, relinkableFrom: true, relinkableTo: true, reshapable: true, resegmentable: true },
        $(go.Shape, { stroke: '#6b7280' },
          new go.Binding('strokeDashArray', 'kind', (k: string) => k === 'aggregation' ? [6, 4] : null)),
        $(go.Shape, { toArrow: 'Standard', stroke: null, fill: '#6b7280' },
  ...
  private onSelectionChanged() {
    ...
    if (!sel) { this.selectedType.set(null); this.selectedNodeData = undefined; this.selectedLinkData = undefined; return; }
    if (sel instanceof go.Node) {
      this.selectedType.set('node');
      this.selectedNodeData = sel.data;
      const attrs: EntityAttr[] = (sel.data.attrs ?? []).map((a: any) => ({ name: a.name, type: a.type, pk: !!a.pk, unique: !!a.unique, nullable: !!a.nullable }));
  ...
  addAttr() { this.attrsFA().push(this.fb.group({ name: ['field', Validators.required], type: ['string', Validators.required], pk: [false], unique: [false], nullable: [true] })); }
  removeAttr(i: number) { this.attrsFA().removeAt(i); }
  ...
  addClass() {
    ...
    m.addNodeData({ key, name: 'Clase', stereotype: 'entity', attrs: [{ name: 'id', type: 'uuid', pk: true, nullable: false }] });
    const n = this.diagram.findNodeForKey(key); if (n) n.location = p;
    this.diagram.commitTransaction('new-class');
  }
  layout() { (this.diagram.layout as any)?.doLayout(this.diagram); }
  undo() { (this.diagram.undoManager as any).undo(); }
  redo() { (this.diagram.undoManager as any).redo(); }
  canUndo() { return (this.diagram.undoManager as any)?.canUndo(); }
  canRedo() { return (this.diagram.undoManager as any)?.canRedo(); }

  // ------------- Load/Save
  private toDSL(): DSL {
    const m = this.diagram.model as go.GraphLinksModel;
    const entities: Entity[] = (m.nodeDataArray as any[]).map(n => ({
      name: n.name, stereotype: n.stereotype ?? '', attrs: (n.attrs ?? []) as EntityAttr[]
    }));
  ...
  private fromDSL(dsl: DSL) {
    const nodeData = dsl.entities.map(e => ({ key: e.name, name: e.name, stereotype: e.stereotype ?? '', attrs: e.attrs ?? [] }));
    const linkData = dsl.relations.map((r, i) => ({ key: `L${i}`, from: r.from, to: r.to, kind: r.kind ?? 'association', fromCard: r.fromCard ?? 'N', toCard: r.toCard ?? '1' }));
    const m = this.diagram.model as go.GraphLinksModel;
  ...
  save(message = 'edit') {
    ...
    this.api.save(this.projectId(), { branchId: this.branchId(), message, content: dsl })
      .subscribe({
